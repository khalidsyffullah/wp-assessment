...
class ACF_Block implements ACF_Blocks_Interface {
    ...
    public function register_block($block_slug = "") {
        ...
        wp_register_style('lh-acf-block-' . $block_slug . '-editor-css',
            $relative_path_between_wp_root_and_active_theme . 'assets/dist/css/blocks/' . $block_slug . '-editor.min.css',
            [],
            LH_PLUGIN_VERSION,
            'screen');

        wp_register_script('lh-acf-block-' . $block_slug . '-editor-js',
    ...
    static function start_timer($function = 'render_block') {
        if (current_user_can('administrator')) { apply_filters('timer:start', static::$slug . ' - ' . $function); }
    }
    ...
    static function stop_timer($function = 'render_block') {
        if (current_user_can('administrator')) { apply_filters('timer:stop', static::$slug . ' - ' . $function); }
    }
